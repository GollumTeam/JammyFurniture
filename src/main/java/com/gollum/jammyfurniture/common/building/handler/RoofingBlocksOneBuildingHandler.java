...
public class RoofingBlocksOneBuildingHandler extends BuildingBlockHandler {
	
	@Override
	protected boolean mustApply (World world, int x, int y, int z, Unity unity) {
		return unity.block instanceof RoofingBlocksOne;
	}
	...
	protected int applyMetadata(World world, int x, int y, int z, int metadata, Unity unity, int rotate) {
		...
		int subBlock = ((RoofingBlocksOne)unity.block).getEnabledMetadata(metadata);
